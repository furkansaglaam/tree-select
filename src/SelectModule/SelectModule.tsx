...
const NestedSelect = ({
  ...
  useEffect(() => {
    selectedValue.map((country: any) => {
      ...
      if (country?.subData?.length > 0) {
        country.subData.map((zone: any) => delete zone.disabled);
      }
    });
  ...
  const openShow = (e: any, i: number) => {
    if (i === expandCountry) setExpandCountry(-1);
    else setExpandCountry(i);
  };
  ...
  const searchCountiresorState = (e: any) => {
    ...
    document
      .querySelectorAll(".NSI-select-menuitem-list")
      .forEach((ef: any) => {
        if (ef.id?.includes(val)) {
          var elementToScroll = document.getElementById(ef.id);
          var elementInScroll = document.getElementById(
            "NSI-select-drop-down-menu-itembox"
          );
  ...
  const selecttheCountry = (e: any, c_data?: any) => {
    ...
    if (findInselected(c_data, false)[0]) {
      ...
      if (e.target.checked || !e.target.dataset.checked) {
        ...
        array = checkedValues.filter(
          (item: any) => item.value !== c_data.value
        );
      }
    }
    ...
    if (findInselectedarray(c_data.value, s_state.value, false)[0]) {
      ...
      checkedValues?.forEach((element: any, sd: number) => {
        if (element.value === c_data.value) {
          var finalResult;
          var counrtyFilter;
          var state_present = element.subData.some(
            (item: any) => item.value === s_state.value
          );
          if (state_present) {
            finalResult = element.subData.filter(
              (ele: any) => ele.value !== s_state.value
            );
            element.subData = finalResult;
            if (!(finalResult?.length > 0))
              counrtyFilter = checkedValues.filter(
                (d: any) => element?.value !== d.value
              );
          } else {
            element.subData = [...element.subData, s_state];
          }
          array =
            counrtyFilter?.length > 0 ? [...counrtyFilter] : [...checkedValues];
          // array = [...checkedValues];
        } else {
          if (count === 0) {
            const option = {
    ...
    checkedValues.forEach((element: any) => {
      if (element.value === country_data.value) {
        ...
        if (element?.subData?.length === country_data?.subData?.length) {
          result[0] =
            element?.subData?.length === country_data?.subData?.length;
          // if(element?.disabled){ //@@
          //     result.push(true);
          // }else{//@@
          //     result.push(false);
  ...
  const getValue = (ctx: any) => {
    ...
    if (ctx) {
      checkedValues?.map((d: any) => {
        var s: string =
          d.subData.length > 0 ? `(${d.count} of ${d.subData.length}),` : ",";
  ...
  return (
    <div className="NSI-main-wrapper" ref={ref} style={{ width: width }}>
      <div
        className={`${inputClass} NSI-input-box-wrap`}
            ...
            {checkedValues?.length > 0 && (
              <div className="NSI-main-overlap-nested">
                {checkedValues?.map(
                  (item: any, i: number) =>
                    i < chipNoCount && (
                      <span className="NSI-main-chipicons-label">
                        {item.name}
                        {item.subData.length > 0 && (
                          <>
                            ({item.subData.length} of{" "}
                            {item.subData.length ?? "1"} )
                          </>
                        )}{" "}
                        <img
                          src={Close}
                          style={{ width: 8, marginLeft: 4, cursor: "pointer" }}
                          onClick={() => chipDelete(item)}
                        />
                      </span>
                    )
                )}
                {checkedValues?.length > chipNoCount && (
                  <span
                    className="NSI-main-more-trailing"
                    onClick={() => {
                      if (onViewmore) {
                        setToggleView(true);
                  ...
                  >
                    {" "}
                    + {checkedValues?.length - chipNoCount} more
                  </span>
                )}
                {checkedValues?.length == chipNoCount &&
                  chipExpandView &&
      ...
      {helperText && (
        <p
          className={
            error
              ? "NSI-main-input-helper-text NSI-error-text"
              : "NSI-main-input-helper-text"
          }
        >
          {helperText}
        </p>
        ...
        {openDropDown && (
          <div className={`${dropDownClass} NSI-select-drop-down-menu-wrapper`}>
            {showContinent == true ? (
              <>
                <div
                  className="NSI-select-drop-down-menu-itembox"
                  id="NSI-select-drop-down-menu-itembox"
                  style={{ height: height }}
                >
                  ...
                  {Countries(showCustomList, showContinent).map(
                    (conti_data: any, index: number) => (
                      <div key={index}>
                        {showContinent && (
                          <div
                            className="NSI-continent-listitem"
                            id={conti_data.name?.toLowerCase()}
                            key={index}
                          >
                        ...
                        {/* {conti_data?.countries.map((c_data: any, i: number) =>
                                            <div key={i} className={expandCountry === i ? "NSI-select-drop-down-menu-expanded" : ""} data-id={i} style={c_data?.disabled ? { background: "#cccccc79" } : {}}>
                                                ...
                                                {showState && expandCountry === i && c_data.subData.length > 0 &&
              ...
              <>
                <div
                  ...
                  style={{ height: height }}
                >
                  {allOptionSelectable == true && (
                    <div>
                      {
                        <li
                          className="NSI-select-menuitem-list"
                          onClick={(e: any) => {
                            setselectAllRegions(!selectAllRegions);
                            selectAllCountryState(
                              e,
                              !selectAllRegions,
                              showContinent
                            );
                  ...
                  {Countries(showCustomList, showContinent).map(
                    (c_data: any, index: number) => (
                      <div
                        key={index}
                        className={
                          expandCountry === index
                            ? "NSI-select-drop-down-menu-expanded"
                            : ""
                        }
                        data-id={index}
                        style={
                          c_data?.disabled ? { background: "#cccccc79" } : {}
                        }
                      >
                        {
                          <li
                            className="NSI-select-menuitem-list"
                            id={c_data.name?.toLowerCase()}
                            onClick={(e: any) => {
                              if (
                                !(c_data?.disabled && c_data?.disabled == true)
                              ) {
                          ...
                          >
                            <div className="NSI-select-menuitem-leading">
                              <input
                                ...
                                className="NSI-select-menuitem-checkbox"
                                checked={
                                  findInselected(c_data, c_data?.disabled)[0]
                                }
                                disabled={disableSelectBox || c_data?.disabled}
                                onChange={() => {}}
                                // onChange={(e: any) => selecttheCountry(e, c_data)}
                            ...
                            {c_data?.subData?.length > 0 && showState && (
                              <div className="NSI-select-menuitem-trailing">
                                {showTrailing && (
                                  <span className="NSI-select-menuitem-trailing-text">
                                    {selectedCount(c_data.value)} of{" "}
                                    {c_data?.subData?.length}{" "}
                                  </span>
                                )}
                                <img
                                  src={Arrow}
                                  className={
                                    expandCountry === index
                                      ? "NSI-select-arrow-collapse-svg"
                                      : "NSI-select-arrow-expand-svg"
                                  }
                                  onClick={(e: any) => {
                                    openShow(e, index);
                                    e.stopPropagation();
                        ...
                        {showState &&
                          expandCountry === index &&
                          c_data.subData.length > 0 &&
                          c_data.subData.map((item: any, k: number) => (
                            <li
                              className="NSI-select-menuitem-list inner-list"
                              key={k}
                              onClick={(e: any) => {
                                if (!(item?.disabled && item?.disabled == true))
                                  selecttheState(e, c_data, item, k, index);
                              }}
